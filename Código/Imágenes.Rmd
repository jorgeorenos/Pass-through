...
# Tipo de cambio medio y varianza

Se separó la media y varianza del tipo de cambio dependiendo de si fue antes de la implementación de la ley de libre negociación de divisas, luego de la implementación y el último esquema de política cambiaria.

```{r}

tc <- tc %>% mutate(esquema = case_when(Periodo < as.Date("2000-05-01") ~ "luego de la liberación", 
                                        Periodo >= as.Date("2000-05-01") & Periodo < as.Date("2011-01-01") ~             
                                                                                      "implementación ley",
                                        Periodo >= as.Date("2011-01-01") ~ "actual politica cambiaria"))

group_by(tc, esquema) %>% 
  summarise(media = mean(TC), "varianza" = var(TC))
